...
function setToArray(mySet) {
  ...
  // Повертаємо оброблений масив.
	const myArray = [...mySet];
	const stringArray = myArray.filter((element) => typeof element === 'string');
	const sortArr = stringArray.sort();
	return sortArr;
}
...
function iterateSet(set) {
  ...
  // Виведення запису у консоль
	let setIter1 = set.keys();
	for (let key of setIter1) {
		console.log(key);
	}
	let setIter2 = set.values();
	for (let value of setIter2) {
		console.log(value);
	}
	let setIter3 = set.entries();
	for (let obj of setIter3) {
		console.log(obj);
	}
}
